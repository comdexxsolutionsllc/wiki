    ...
    <div class="wiki-side-menu">
        <div class="side-menu-inner">
            <div class="wiki-intro">
                <div class="pull-left">
                    <h1 class="header" style="position: relative; top: 4px;"><a
                                href="{{ route('wikis.show', [$team->slug, $space->slug, $wiki->slug, ]) }}">{{ $wiki->name }}</a>
                    </h1>
                </div>
                <div class="pull-right wiki-like-con">
                    <i class="fa fa-spinner fa-spin fa-lg fa-fw" id="spinner"></i>
                    <a href="#" id="like-wiki" data-wiki="{{ $wiki->slug }}"><i class="fa fa-star-o fa-fw"
                                                                                data-toggle="tooltip"
                                                                                data-placement="bottom"
                                                                                title="{{ $isUserLikeWiki ? 'Unlike' : 'Like' }}"
                                                                                style="margin-right: 3px; font-size: 16px;"></i></a>
            ...
            <div class="side-menu-page-tree-list" style="margin-bottom: 15px;">
                <div class="nav-header" style="margin-bottom: 10px;"
                     title="You can move any page by dragging it to a new position in the tree.">Page tree
                </div>
                @if(isset($page))
                    <span class="hidden" id="page-open" data-page="{{ $page->slug }}"></span>
                @endif
                <div id="wiki-page-tree" data-wiki="{{ $wiki->slug }}"
                     @if(isset($page)) data-page="{{ $page->id }}" @endif></div>
            </div>
        </div>
    </div>
</div>
