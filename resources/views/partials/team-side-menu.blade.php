        ...
        <div class="side-menu-wiki-list">
            <ul class="list-unstyled" id="categories-list">
                <li class="nav-header" style="position: relative; margin-bottom: 12px;">
                    ...
                    <a href="{{ route('spaces.create', [ $team->slug ]) }}"
                       style="position: absolute; right: 10px; top: 5px; color: #9c9c9c;">
                ...
                @if($spaces->count() > 0)
                    ...
                    <div class="list">
                        @foreach($spaces as $item)
                            <li class="item {{ isset($space) && ($space->slug === $item->slug) ? 'active' : '' }}"
                                id="categories-list-item" data-name="{{ $item->name }}">
                                <a href="{{ route('spaces.wikis', [$team->slug, $item->slug, ]) }}">
                                    <div class="cateogry-icon"
                                         style="margin-right: 13px; position: relative; top: 1px;"></div>
                ...
                @else
                    <li class="text-center text-muted">
                        <h1 class="nothing-found side" style="margin-top: 20px; "><i
