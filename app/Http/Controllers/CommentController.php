    ...
    public function notifyMentionedUsers($mentions, $wiki, $page = null)
    {
        foreach ($mentions as $mention) {
            ...
            if (is_null($page)) {
                ...
                Notifynder::category('wiki.user.mentioned')->from(Auth::user()->id)->to($user->id)->url($url)->extra([
                    'wiki_name' => $wiki->name,
                ...
                Notifynder::category('page.user.mentioned')->from(Auth::user()->id)->to($user->id)->url($url)->extra([
                    'wiki_name' => $wiki->name,
                    'username'  => Auth::user()->name,
                    'page_name' => $page->name,
                ])->send();
