...
class Page extends Node
{
            ...
            if (! empty($watchingList)) {
                foreach ($watchingList as $watch) {
                    $url = route('pages.show', [
                        ...
                        $watch->wiki->space->slug,
                        $watch->wiki->slug,
                        $page->slug,
                    ]);
                    Notifynder::category('page.created')->from(Auth::user()->id)->to($watch->user_id)->url($url)->extra([
                        'wiki_name' => $watch->wiki->name,
        ...
        static::updated(function ($page) {
            ...
            if (! empty($watchingList)) {
                foreach ($watchingList as $watch) {
                    ...
                    Notifynder::category('page.updated')->from(Auth::user()->id)->to($watch->user_id)->url($url)->extra([
                        'wiki_name' => $watch->wiki->name,
                        'username'  => Auth::user()->name,
                        'page_name' => $page->name,
                    ])->send();
        ...
        static::deleting(function ($page) {
            ...
            if (! empty($watchingList)) {
                foreach ($watchingList as $watch) {
                    ...
                    Notifynder::category('page.deleted')->from(Auth::user()->id)->to($watch->user_id)->url($url)->extra([
                        'wiki_name' => $watch->wiki->name,
                        'username'  => Auth::user()->name,
                        'page_name' => $page->name,
                    ])->send();
    ...
    {
        $page = $this->where('slug', '=', $slug)->where('user_id', '=', Auth::user()->id)->with([
            'comments',
            'wiki',
        ])->first();
        if (is_null($page)) {
