...
class Wiki extends Model
{
        ...
        static::updated(function ($wiki) {
            ...
            if (! empty($watchingList)) {
                foreach ($watchingList as $watch) {
                    ...
                    Notifynder::category('wiki.updated')->from(Auth::user()->id)->to($watch->user_id)->url($url)->extra([
        ...
        static::deleting(function ($wiki) {
            ...
            if (! empty($watchingList)) {
                foreach ($watchingList as $watch) {
                    ...
                    Notifynder::category('wiki.deleted')->from(Auth::user()->id)->to($watch->user_id)->url($url)->extra([
                        'wiki_name' => $watch->wiki->name,
                        'username'  => Auth::user()->name,
                    ])->send();
                }
    ...
    {
        return $this->hasMany(Comment::class, 'subject_id', 'id')->where('comments.subject_type', Wiki::class)->with([
            'user',
            'likes',
        ]);
    }
